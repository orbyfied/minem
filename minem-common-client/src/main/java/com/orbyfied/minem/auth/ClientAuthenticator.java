...
public class ClientAuthenticator extends ClientComponent {
    ...
    public static ClientAuthenticator create() {
    ...
    @Getter
    AuthenticationContext accountContext;
    ...
    public ClientAuthenticator accountContext(AuthenticationContext accountContext) {
    ...
    protected boolean attach(MinecraftClient client) {
        ...
        client.onStateSwitch().addLast((oldState, newState) -> {
            if (newState == ClientState.LOGIN) {
                ...
                client.getConnection().sendSync(client.createPacket("ServerboundLoginStart",
    ...
    private int onPacketReceived(PacketContainer pc) {
        // encryption request
        if (pc.data() instanceof ClientboundEncryptionRequestPacket) {
            try {
                final ClientboundEncryptionRequestPacket packet = pc.data();
                ...
                client.getConnection().sendSync(client.createPacket("ServerboundEncryptionResponse",
                ...
                client.getConnection().withCiphers(encryptionCipher, decryptionCipher);
            } catch (Exception ex) {
        ...
        // success packet
        if (pc.data() instanceof ClientboundLoginSuccessPacket) {
            final ClientboundLoginSuccessPacket packet = pc.data();
    ...
    public interface ClientLoginCompletedHandler {
        void onLoginComplete(MinecraftClient client,
                             ...
                             PacketContainer acknowledgedPacket);
    }
