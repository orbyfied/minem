...
public class MutablePacketRegistry implements PacketRegistry {
    ...
    public void match(Collection<PacketMapping> list, Object key) {
        if (key instanceof String) {
            PacketMapping byAlias = mappingsByAlias.get((String) key);
            if (byAlias != null) {
                list.add(byAlias);
            }
        } else if (key instanceof Class<?>) {
            PacketMapping byClass = mappingsByClass.get((Class<?>) key);
            if (byClass != null) {
                list.add(byClass);
                return;
            }
            ...
            for (PacketMapping mapping : mappingList) {
                if (mapping.getDataInterfaces().contains((Class<?>) key)) {
                    list.add(mapping);
                }
            }
        }
