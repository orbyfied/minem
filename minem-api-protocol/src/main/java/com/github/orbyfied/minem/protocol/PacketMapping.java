...
public class PacketMapping {
    ...
    public String toString() {
        return "PacketMapping(named: " + primaryName + ", declared by " + declaringClass.getSimpleName() + " with data class: " + dataClass.getSimpleName() + ")";
    }

    public void writePacketData(Packet packet, UnsafeByteBuf buf) {
        try {
    ...
    public void readPacketData(Packet packet, UnsafeByteBuf buf) {
        try {
     ...
     * @return The packet container.
     */
    public Packet createPacketContainerWithData(ProtocolContext context) {
        try {
    ...
    public static PacketMapping compileDataMapping(Class<?> klass, Mapping annotation) {
        try {
            ...
            var mType = MethodType.methodType(void.class, Packet.class, UnsafeByteBuf.class);
     ...
     * Compile the given class to a packet mapping from the unique data class using:
     * - The {@link Mapping} annotation,
     * - The methods in the {@link PacketData} interface statically implemented and
    ...
    public static PacketMapping compileStaticMapping(Class<?> klass, Mapping annotation) {
        try {
            ...
            MethodHandle methodRead = lookup.findStatic(klass, "read", MethodType.methodType(void.class, dataClass, Packet.class, UnsafeByteBuf.class));
            MethodHandle methodWrite = lookup.findStatic(klass, "write", MethodType.methodType(void.class, dataClass, Packet.class, UnsafeByteBuf.class));
