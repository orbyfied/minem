...
public class SecretStore {
    ...
    @SuppressWarnings("unchecked")
    public <T, S extends Secret<T>> S getSecretOrNull(String name) {
        return (S) secretMap.get(name.toLowerCase());
    }
    ...
    public SecretStore storeToken(String name, String value, long expiry) {
        return storeSecret(new Token(name).value(value).timeObtained(System.currentTimeMillis()).expiry(expiry));
    }
